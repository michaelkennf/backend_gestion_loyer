...
import { PrismaClient, Role, PropertyType, ExpenseType } from "@prisma/client";
...
async function propertyExists(propertyType: "house" | "studio", propertyId: string) {
...
app.post("/api/payments", auth, allow(Role.MANAGER), async (req: AuthRequest, res) => {
  const schema = z.object({
    propertyType: z.enum(["house", "studio"]),
...
app.post("/api/expenses", auth, allow(Role.MANAGER), async (req: AuthRequest, res) => {
  ...
  const expense = await prisma.expense.create({
    data: {
      expenseType: parsed.data.expenseType === "common" ? ExpenseType.COMMON : ExpenseType.PRIVATE,
      propertyType: isHouse ? PropertyType.HOUSE : PropertyType.STUDIO,
      houseId: isHouse ? parsed.data.propertyId : null,
      studioId: isHouse ? null : parsed.data.propertyId,
...
app.get("/api/dashboard", auth, async (_req: AuthRequest, res) => {
  ...
  const expenseDto = expenses.map((e) => ({
    ...
    propertyType: e.propertyType === PropertyType.HOUSE ? "house" : "studio",
